...
EXP="${TASK}-${MODEL_ID}-${EXPERIMENT}-lr${LR}-taus${ANNEAL_START_TAU}to${ANNEAL_END_TAU}in${ANNEAL_EPOCHS}epochs-rollout${ROLLOUT_N}-ttr${ENABLE_TRAIN_TEMP}-${DATE}-${EPOCHS}epoch"
OUTPUT_DIR="${ROOT_DIR}/outputs/${MODEL}/${TASK}/${EXPERIMENT}/lr${LR}-taus${ANNEAL_START_TAU}to${ANNEAL_END_TAU}in${ANNEAL_EPOCHS}epochs-rollout${ROLLOUT_N}-ttr${ENABLE_TRAIN_TEMP}/${DATE}"
...
python3 -m recipe.sept.main_sept \
